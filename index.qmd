...
num_features <- c("Age", "CreditScore", "Tenure", "Balance", "EstimatedSalary")
df_encoded[num_features] <- scale(df_encoded[num_features])
...
# Split the data
train_index <- createDataPartition(df_encoded$Exited, p = 0.7, list = FALSE)
train_df <- df_encoded[train_index, ]
test_df <- df_encoded[-train_index, ]
```

```{r}
# Create a table of the 'Exited' column
exited_table <- table(train_df$Exited)

# Convert the table to a dataframe for plotting
exited_df <- as.data.frame(exited_table)

# Create a bar plot
ggplot(exited_df, aes(x = Var1, y = Freq)) +
  geom_bar(stat = "identity", fill = "steelblue") +
  labs(title = "Distribution of Exited Customers in train test",
       x = "Exited",
       y = "Frequency") +
...
```{r}
# Create a table of the 'Exited' column
exited_table <- table(test_df$Exited)
...
ggplot(exited_df, aes(x = Var1, y = Freq)) +
  geom_bar(stat = "identity", fill = "steelblue") +
  labs(title = "Distribution of Exited Customers in test set",
       x = "Exited",
       y = "Frequency") +
  theme_minimal() + theme(legend.title = element_blank(),
        plot.title = element_text(hjust = 0.5)) 
